...
public class ResponseTestServlet extends HttpServlet {
    ...
    /* 설명. req, resp는 요청이 들어온 곳의 주소값을 가지고 있음. */
    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {

        /* 설명.
            1. 요청 받기(http method GET/POST 요청에 따른 parameter로 전달 받은 데이터를 꺼낼 수 있다.
            2. 비즈니스 로직 처리(DB 접속과 CRUD에 대한 로직 처리 시작 부분 -> 서비스 계층 호출(MVC2구조 기반)
            3. 응답하기(문자열을 통한 동적인 웹(HTML 페이지)페이지를 만들어서 스트림을 통해 내보내기)(SSR 기술)
            CSR(Client Side): Client에서 페이지 제작  /  SSR(Server Side): Server에서 페이지 제작
         */
        /* 설명. SSR: 서버측에서 화면에 띄울 문자를 만들어 보내는 과정 */
